...
要捕获Crash，我们先需要明确几个概念及其之间的关系：`硬件异常`、`软件异常`、`mach异常`、`Signal信号`，这四种异常概念，自底向上构建了iOS系统的异常处理模型。他们之间的过膝乳腺图所示：
...
> 实际使用中发现，这种方式对于OC的crash有效，对于Swift的crash比如SIGTRAP还是无法进入handler中的断点。因此我们只能通过写log的方式来进行调试。
...
经过解析我们发现第一行和第二行对应的是我们的注册的signalHandler，没有什么用，我们再往下解析第五行就能定位到异常发生的位置了。
